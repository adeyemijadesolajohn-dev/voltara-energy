...
import "./Table.scss";
import { initialColumns, initialRows } from "../../data/PartnersData";
...
const Table = () => {
  ...
  // Debounce filter
  useEffect(() => {
    const handler = setTimeout(() => setDebouncedFilter(filterText), 300);
  ...
  useEffect(() => {
    const handleResize = () => {
      setAnimateDate(true);
      setTimeout(() => setAnimateDate(false), 300);
      setIsCompactView(window.innerWidth <= 1240);
    };
  ...
  const exportToCSV = () => {
    const csv = [
      ...
      ...rows.map((row) =>
        columns
          .map((col) => {
            if (col.field === "topup") return formatDate(row);
            ...
            if (["amount", "balance", "purchase"].includes(col.field)) {
  ...
  const toggleSelectAll = () => {
    ...
    currentPageIds.forEach((id) =>
      selectAll ? updated.delete(id) : updated.add(id)
    );
        ...
        <div className="historyTitle">
          <div className="historyTitleContainer">
            <div className="historyIcon">
              ...
              <Icons.Partnership className="topIcon" />
            </div>

            <h3 className="historyTitleText"> Partners</h3>
          </div>
